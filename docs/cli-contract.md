...
- `coverage: null` forces `crap: null`; nulls occur only when no coverage file was loaded. A function with no coverage entry counts as 0% with a computed CRAP whenever a coverage file is given.
- `--max-crap` exits 2 only when no coverage data was loaded at all.
